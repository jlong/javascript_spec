...
class JavaScriptSpec
    ...
    def setup
      if windows?
        puts %{
          ...
          You have to shut down the Internet Explorer manually after each spec
  ...
  class Runner
    ...
    def define
      ...
      # run all combinations of browsers and specs
      @browsers.each do |browser|
        if browser.supported?
          browser.setup
          @specs.each do |spec|
            browser.visit("http://localhost:4711#{spec}?resultsURL=http://localhost:4711/results&t=" + ("%.6f" % Time.now.to_f))
            result = @queue.pop
            puts "#{spec} on #{browser}: #{result}"
    ...
    # spec should be specified as a url
    def run(spec)
      url = "/spec/javascript/#{spec}_spec.html"
      unless File.exists?(RAILS_ROOT+url)
        raise "Missing spec file #{url} for #{spec}"
      end
      @specs << url
    end
